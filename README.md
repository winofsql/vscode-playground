...
1. ボックス選択 - <span class="mac-only windows-only">テキストブロックを選択するには Shift+DownArrow, Shift+RightArrow, Shift+UpArrow, Shift+LeftArrow のいずれかの組み合わせを</span>押します。<span class="shortcut mac-only">`⇧⌥`</span><span class="shortcut windows-only linux-only">`Shift+Alt`</span> を押しながらマウスでテキストを選択することもできます。
